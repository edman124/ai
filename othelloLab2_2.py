...
def main():
    ...
    try:
        if len(sys.argv)==1:
            ...
            printStats(board)
            exit(0)
        elif len(sys.argv[checki]) > 2:
            board = sys.argv[checki].lower()
            checki+= 1
            if len(sys.argv)==2:
                formatPrint(board)
                printStats(board)
                exit(0)
            if len(sys.argv) > checki:
                if ifInt(sys.argv[checki]) or len(sys.argv[checki]) > 1:
                    turn = curTurn(board)
                    ind.append(sys.argv[checki])
                    checki +=1
                    while len(sys.argv) > checki:
                        ind.append(sys.argv[checki])
                        checki +=1
                else:
                    turn = sys.argv[checki].lower()
                    checki += 1
                    if len(sys.argv) > checki:
                        ind.append(sys.argv[checki])
                        checki+=1
        ...
        for i in ind:
            if not ifInt(i):
                i = col[i[0].lower()] + (int(i[1]) - 1) * 8
            i = int(i)
            moves.append(i)
            possmoves = findMoves(board, turn)
            board = updateBoard(board,possmoves,i)
            if '*' not in board and '+' not in board:
                turn = nextTurn(turn)
                possmoves = findMoves(board, turn)
                board = updateBoard(board, possmoves, i)
            formatPrint(board)
